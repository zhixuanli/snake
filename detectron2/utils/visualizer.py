...
class Visualizer:
    def __init__(self, img_rgb, metadata, scale=1.0, instance_mode=ColorMode.IMAGE,
                 edge_width=1, ifDrawBox=True, ifShowLabel=True):
        """
        Args:
        ...
        self.ifDrawBox = ifDrawBox
        self.ifShowLabel = ifShowLabel
    ...
    def draw_instance_predictions(self, predictions, alpha=0.5):
        """
    ...
    def draw_dataset_dict(self, dic, alpha=0.5):
        """
        ...
        if annos:
            ...
            if self.ifShowLabel:
                ...
                labels = None
            if self.ifDrawBox is False:
                boxes = None
            self.overlay_instances(labels=labels, boxes=boxes, masks=masks, keypoints=keypts, alpha=alpha)

        sem_seg = dic.get("sem_seg", None)
        if sem_seg is None and "sem_seg_file_name" in dic:
            sem_seg = cv2.imread(dic["sem_seg_file_name"], cv2.IMREAD_GRAYSCALE)
        if sem_seg is not None:
            self.draw_sem_seg(sem_seg, area_threshold=0, alpha=alpha)
        return self.output
    ...
    def draw_polygon(self, segment, color, edge_color=None, alpha=0.5):
        ...
        polygon = mpl.patches.Polygon(
            ...
            edgecolor=edge_color,
            linewidth=max(self._default_font_size // 15 * self.output.scale, self.edge_width),
        )
