...
def change_to_instance_index(origin_list):
    ...
    for i in range(len(origin_list)):
        ...
        for k in range(instance_count):
            ...
            px = seg_poly[::2]
            py = seg_poly[1::2]
            instance_anno_["bbox"] = [np.min(px), np.min(py), np.max(px), np.max(py)]
...
if __name__ == '__main__':
    ...
    # train
    annotation_file = dataset_root + "COCO_amodal_2014/annotations/COCO_amodal_train2014.json"
    coco_new_path = "cocoa_train.json"
    trans_to_COCO_format(annotation_file, coco_new_path)

    # val
    annotation_file = dataset_root + "COCO_amodal_2014/annotations/COCO_amodal_val2014.json"
